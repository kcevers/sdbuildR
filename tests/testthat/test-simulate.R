...
test_that("simulate() result has correct structure", {
  ...
  expect_true("df" %in% names(result))
  expect_true("sfm" %in% names(result))
...
test_that("simulate() with multiple stocks", {
  sfm <- sdbuildR()
  sfm <- build(sfm, "S", type = "stock", eqn = "100")
  sfm <- build(sfm, "I", type = "stock", eqn = "10")
  sfm <- build(sfm, "infection", type = "flow", from = "S", to = "I", eqn = "0")
...
test_that("simulate() with graphical function dependency", {
  ...
  sfm <- build(sfm, "Stock1", type = "stock", eqn = "50")
  sfm <- build(sfm, "Flow1", type = "flow", from = "Stock1", eqn = "gf1(Stock1)")
  sfm <- sim_specs(sfm, language = "R", start = 0, stop = 10, dt = 1)

  expect_successful_simulation(sfm)

  # Without source
  sfm <- build(sfm, "gf1", source = NA)
