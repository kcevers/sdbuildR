...
test_that("dependencies maps basic cross-type dependencies", {
  sfm <- sdbuildR()
  sfm <- build(sfm, "c1", type = "constant", eqn = "5")
  sfm <- build(sfm, "S", type = "stock", eqn = "c1")
  sfm <- build(sfm, "A", type = "aux", eqn = "S + c1")
  sfm <- build(sfm, "F1", type = "flow", from = "S", eqn = "A")
...
test_that("dependencies includes gf source dependencies", {
  sfm <- sdbuildR()
  sfm <- build(sfm, "S", type = "stock", eqn = "10")
  # Graphical function with source=S
  sfm <- build(sfm, "G", type = "lookup", source = "S", xpts = c(0, 10), ypts = c(0, 100))
...
test_that("dependencies_ includes func names with only_model_var=TRUE", {
  sfm <- sdbuildR() |>
    ...
    build("x", "constant", eqn = "1") |>
    build("A", "aux", eqn = "mfun(x)")
...
test_that("dependencies_ ignores names inside quotes", {
  sfm <- sdbuildR() |>
    build("Stock1", "stock", eqn = "10") |>
    build("A", "aux", eqn = "paste('Stock1')")
...
test_that("order_equations() detects circular dependencies", {
  sfm <- sdbuildR()
  sfm <- build(sfm, "a", type = "constant", eqn = "b")
  suppressWarnings(sfm <- build(sfm, "b", type = "constant", eqn = "a") |>
  build("Stock1", type = "stock") |> build("Flow1", type = "flow", from = "Stock1", eqn = "0"))
...
test_that("order_equations() warns for circular dependencies in static part", {
  ...
  sfm <- build(sfm, "constant1", type = "constant", eqn = "constant2")
  suppressWarnings(sfm <- build(sfm, "constant2", type = "constant", eqn = "constant1"))
...
test_that("order_equations() warns for circular dependencies in aux", {
  sfm <- sdbuildR()
  sfm1 <- build(sfm, "aux1", type = "aux", eqn = "aux2")
  suppressWarnings(sfm2 <- build(sfm1, "aux2", type = "aux", eqn = "aux1"))
