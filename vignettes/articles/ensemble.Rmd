...
```{r}
plot(sims, i = 1:15, j = 1:2, which = "sims", nrows = 1)
```
...
- `df`: individual simulation data (if `return_sims = TRUE`)
- `init`: initial values of stocks
- `constants`: parameter values used
- `conditions`: matrix showing parameter combinations for each $j$

You can access the summary statistics per condition $j$ and per time point, such as the mean and confidence intervals, using:
...
If you have set `return_sims = TRUE`, you can access the individual simulation runs as well. The dataframe contains the value of each variable, for each time point, for each simulation $i$, per condition $j$. 
